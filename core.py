...
from flask import Flask, render_template, send_from_directory, redirect, redirect, url_for
import os
...
def rotate(angle, filename):
    ...
    return redirect(url_for('banding.html', x="temp.png", ))
...
def Noise(filename):
    ...
    f = f[230:290, 220:320]
    noisy = target + 0.4 * f.std() * np.random.random(f.shape)
    
    return send_image(noisy)
